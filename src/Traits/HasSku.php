...
trait HasSku
{
    ...
    {
        return $this->morphMany(config('tmaic-sku.models.Attr'), config('tmaic-sku.tmaic_name'));
    }
    ...
    public function skus(): MorphMany
    {
        ...
        return $this->morphMany(config('tmaic-sku.models.Sku'), config('tmaic-sku.tmaic_name'));
    }
    ...
    public function syncAttrValues($option, ...$values)
    {
        ...
        $attrClass = config('tmaic-sku.models.Attr');
    ...
    public function addAttrValues($option, ...$values)
    {
        ...
        $values = collect($values)->flatten()->map(function ($value) use ($option) {
            $attribute_class = config('tmaic-sku.models.Attr');
    ...
    public function syncSkuWithAttrs(array $position, array $payload)
    {
        ...
        $attrModel = config('tmaic-sku.models.Attr');
        // 各属性值的选项id
    ...
    {
        $optionModel = config('tmaic-sku.models.Option');
